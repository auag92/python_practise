function[K,V,Q,T,Dfront,OP,flag]=objective(K,V,Q,T,Dfront,OP)

if Q(K,2)==9999 
    V=not(V);
%     V1=not(V);
    obj=[K V];
    flag=1; %in case of NO Dfrontier
else
    ...
    if strcmp(T(h,1),'INV')||strcmp(T(h,1),'BUF')%%THIS CASE NEVER COMES
        if (Q(T{h,2},2)==9999) || (Q(T{h,2},2)==666) %check if value is undefined
            J=T{h,2};
            C=V;
        end
    else strcmp(T(h,1),'OR')||strcmp(T(h,1),'AND')||strcmp(T(h,1),'NOR')||strcmp(T(h,1),'NAND')
        if (Q(T{h,2},2)==9999) || (Q(T{h,2},2)==666)
            J=T{h,2};
        else %Q(T{h,3},2)==9999
            J=T{h,3};
        end
